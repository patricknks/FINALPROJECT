...
PID mypid_yaw           (0.03125, 3.0, -3.0, 0.300, 0.050, 0.00000);
...
void MyDrone::take_off(){
    ...
    while(ros::ok() && (current_state.mode == "OFFBOARD" && current_state.armed) && !is_takeoff){
        ...
        if (current_position.pose.position.z <= 0.3){
            ...
            vel.linear.z    = mypid_z_takeoff.calculate(3, current_position.pose.position.z); //nanti coba ubah pake lidar
        }
        else if(current_position.pose.position.z >= 0.301){
            vel.linear.x    = mypid_x.calculate(start_x, current_position.pose.position.x);
            vel.linear.y    = mypid_y.calculate(start_y, current_position.pose.position.y);
            vel.linear.z    = mypid_z_takeoff.calculate(5, current_position.pose.position.z); //nanti coba ubah pake lidar    
        }
        ...
        if (current_position.pose.position.z <= 2.7 ){  //ini juga ubah pake lidar
            ROS_INFO("Altitude = %f Yaw = %f", current_position.pose.position.z, compass_tf(compass_val.data));
    ...
    while(ros::ok() && current_state.armed){
        if(which_object == "outter"){
            ...
            if (is_middle_detected.data == "true"){
                // waktu1 = ros::Time::now();
                // if ((waktu2 - waktu1) >= 0.5){
                //     waktu2 = ros::Time::now();
                // }        buat bikin skala kompensasi
                // rencana, ambil nilai x1(jarak antara drone ke target) pada detik saat ini dan x2 pada detik ke 0.5,
                // lalu dari situ bisa didapetin kecepatan perubahan nilai x
                // setelah itu nnti dibikin persentase untuk dikalikan ke parameter PID
                centering_outter("local", 2.8);
            }
            else{ 
...
void MyDrone::keep_in_place(double alt){
    ...
    vel.linear.y        = mypid_y.calculate(0.0, current_position.pose.position.y);
    vel.linear.z        = mypid_z.calculate(alt, current_position.pose.position.z);
    if (compass_tf(compass_val.data) >= (0.0 + 0.001)){
        vel.angular.z   = -1 * pidyaw.calculate(0.0001, compass_tf(compass_val.data));
    } 
    ...
    vel.linear.z = 0;
    vel.angular.z = 0;
    ROS_INFO("yaw = %f", compass_tf(compass_val.data));
}
