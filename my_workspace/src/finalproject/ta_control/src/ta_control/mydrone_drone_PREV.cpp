...
MyDrone::MyDrone(double pixel_w, double pixel_h, double hfov, double vfov, double Kp, double Ki, double Kd, std::string object) : rate(32){
    ...
    compass_sub         = nh.subscribe<std_msgs::Float64>               ("mavros/global_position/compass_hdg", 20, &MyDrone::compass_cb, this);
...
void MyDrone::take_off(){
    ...
    while(ros::ok() && (current_state.mode == "OFFBOARD" && current_state.armed) && !is_takeoff){
        ...
        if(((tfmini_seq_now > tfmini_seq_past) && (tfmini_val.range < 7.0)) || (tfmini_seq_now == 0 && tfmini_seq_past == 0)){

            tfmini_seq_past = tfmini_val.header.seq;

            if (tfmini_val.range <= 0.2){
                ...
                vel.linear.z    = mypid_z_takeoff.calculate(5, tfmini_val.range); 
            }
            else if(tfmini_val.range >= 0.301){
                vel.linear.x    = mypid_x.calculate(start_x + 2.0, current_position.pose.position.x);
                vel.linear.y    = mypid_y.calculate(start_y, current_position.pose.position.y);
                vel.linear.z    = mypid_z_takeoff.calculate(5, tfmini_val.range);    
            }
            ...
            if (tfmini_val.range <= 2.8){  
                ROS_INFO("Altitude = %f Yaw = %f", tfmini_val.range, compass_val.data);
    ...
    while(ros::ok() && (current_state.mode == "OFFBOARD" && current_state.armed)){
        ...
        if(((tfmini_seq_now > tfmini_seq_past) && (tfmini_val.range < 7.0)) || (tfmini_seq_now == 0 && tfmini_seq_past == 0)){
            ...
            if (is_middle_detected.data == "true"){
                double x = estimate_x(middle_pixel_val.point.y * 1.53846154, tfmini_val.range);
                double y = estimate_y(middle_pixel_val.point.x * 1.15384615, tfmini_val.range);
                
                if((x <= -0.2) || (x >= 0.2) || ((y <= -0.3) || (y >= 0.3))){
                    centering_outter("tfmini", 3.0);
            ...
            else if((ros::Time::now() - cek_waktu >= ros::Duration(3.0)) && (is_outter_detected.data == "false")){ 
                keep_in_place(3.0);
            }
            else{
                hunting(3.0);
...
void MyDrone::mission(){
    ...
    while(ros::ok() && (current_state.mode == "OFFBOARD" && current_state.armed)){
        ...
        if(((tfmini_seq_now > tfmini_seq_past) && (tfmini_val.range < 5.0)) || (tfmini_seq_now == 0 && tfmini_seq_past == 0)){
            ...
            if ((tfmini_val.range >= 2.5001) && (tfmini_val.range <= 3.5)){
                ROS_INFO("---- misi altitude 2.5 - 3.5 ----");
                if(is_middle_detected.data == "true"){
                ...
                if((ros::Time::now() - cek_waktu) <= ros::Duration(2.0)){
                        ROS_INFO("Hunting OUTTER");
                        // hunting(2.2);
                    }
                    else{keep_in_place(2.2);}
                } 
            }
            
            else if ((tfmini_val.range >= 1.7001) && (tfmini_val.range <= 2.5)){
                ROS_INFO("---- misi altitude 1.7 - 2.5 -----");
                if (is_middle_detected.data == "true"){
                    ...
                    if((ros::Time::now() - cek_waktu) <= ros::Duration(2.0)){
                        ROS_INFO("Hunting MIDDLE");
                        // hunting(1.2);
                    }
                    else{keep_in_place(1.2);
                    }
                }
            }

            else if ((tfmini_val.range >= 0.8001) && (tfmini_val.range <= 1.7)){
                ROS_INFO("---- misi altitude 0.8 - 1.7 ----");
                if (is_inner_detected.data == "true"){
                    ...
                    if((ros::Time::now() - cek_waktu) <= ros::Duration(2.0)){
                        ROS_INFO("Hunting INNER");
                        // hunting(0.7);
                    }
                    else{keep_in_place(0.7);
                    }
                }
            }
                
            else if ((tfmini_val.range >= 0.1001) && (tfmini_val.range <= 0.8)){
                ROS_INFO("---- misi altitude <= 0.8 ----");
                if (is_inner_detected.data == "true"){
                    centering_inner2("tfmini", 0.001);
                    cek_waktu = ros::Time::now();
                }
                else {
                    if((ros::Time::now() - cek_waktu) <= ros::Duration(2.0)){
                        ROS_INFO("Hunting INNER2");
                        // hunting(0.5);
                    }
                    else{keep_in_place(0.5);
                    }
                }
            }
...
void MyDrone::centering_outter(std::string sensorType, double desired_alt){
    ...
    vel.linear.y    = -1 * mypid_y_vision_outter.calculate(-0.2 + y + current_position.pose.position.y, current_position.pose.position.y);    
    ...
    vel_pub.publish(vel);
    ROS_INFO("Centering OUTTER x = %f    y = %f    z = %f", x, y, alt);
}
...
void MyDrone::centering_middle(std::string sensorType, double desired_alt){
    ...
    vel.linear.y    = -1 * mypid_y_vision_middle.calculate(-0.2 + y + current_position.pose.position.y, current_position.pose.position.y);    
    vel.linear.z    = mypid_z.calculate(desired_alt, alt);
    if (compass_tf(compass_val.data) >= (0.0 + 0.001)){
        vel.angular.z   = -1 * pidyaw.calculate(0.0001, compass_tf(compass_val.data));
    } 
    ...
    vel_pub.publish(vel);
    ROS_INFO("Centering MIDDLE x = %f    y = %f    z = %f", x, y, alt);
}
...
void MyDrone::centering_inner(std::string sensorType, double desired_alt){
    ...
    vel.linear.y    = -1 * mypid_y_vision_inner.calculate(-0.15 + y + current_position.pose.position.y, current_position.pose.position.y);    
    vel.linear.z    = mypid_z.calculate(desired_alt, alt);
    if (compass_tf(compass_val.data) >= (0.0 + 0.001)){
        vel.angular.z   = -1 * pidyaw.calculate(0.0001, compass_tf(compass_val.data));
    } 
    ...
    vel_pub.publish(vel);
    ROS_INFO("Centering INNER x = %f    y = %f    z = %f", x, y, alt);
}
...
void MyDrone::centering_inner2(std::string sensorType, double desired_alt){
    ...
    vel_pub.publish(vel);
    ROS_INFO("Centering INNER2 x = %f    y = %f    z = %f", x, y, alt);
}
